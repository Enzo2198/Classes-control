  ...
  implements AnswerServiceI
{
  ...
  async create(answerReq: AnswerReqI): Promise<AnswerResI> {
    const {question_exam_id, answer} = answerReq;

    const query = this.repository.manager
      .createQueryBuilder()
      .select('q.id', 'id')
      .addSelect('q.type', 'type')
      .addSelect('q.correct.answer', 'correct_answer')
      .from('question_exam', 'qe')
      .innerJoin('question', 'q', 'q.id = qe.question_id')
      .where('qe.id = :questionExamId', {questionExamId: question_exam_id})

    const question = await query.getRawOne();
    if(!question) throw new InternalServerErrorException(`Question ${question_exam_id} not found`);
  ...
  async updateMany(answers: AnswerReqI[]): Promise<AnswerResI[]> {
    ...
    const answersData: string = answers.map((a: AnswerReqI) => {
      const { id, question_exam_id, answer, is_correct } = a;
      ...
      return `(${id}, ${question_exam_id}, '${escapeLiteral(answer)}', ${pgArray}, ${userId})`;
    }).join(',');


    const query =
      `
        WITH answers_to_update(id, question_exam_id, answer, is_correct, updated_by) AS (
            VALUES ${answersData}
        )
